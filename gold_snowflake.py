"""
Gold AI Trading Agent PRO v4 — ETF Volume Flow Edition
=========================================================
Features: Live COMEX Gold · Pro Technical Indicators · AI Analysis
5-Model Price Forecasting · Risk Analytics · Correlation
Gold/Silver Ratio · DXY Overlay · News Sentiment · Enhanced UI
ETF Volume Flow: GLD · IAU · SGOL · UGL · GLL
"""
...
st.set_page_config(
    page_title="Gold AI Trading Agent PRO v4",
    page_icon="🥇",
    layout="wide",
...
def signal_score(snap):
    ...
    if gs_ratio > 0:
        if gs_ratio > 80: add("G/S Ratio","Ratio="+str(round(gs_ratio,1)), False, True)
        elif gs_ratio < 60: add("G/S Ratio","Ratio="+str(round(gs_ratio,1)), True, False)
...
def fetch_gs_ratio_and_dxy():
    result = {"silver_price": None, "dxy": None, "yield_10y": None, "gs_ratio": None,
              "silver_daily": None, "dxy_daily": None}
    try:
        silver_df = _yf_fetch(SILVER_TICKER, "1d", "6mo")
        result["silver_price"] = float(silver_df["Close"].iloc[-1])
        result["silver_daily"] = silver_df
    except Exception:
        pass
    try:
...
@st.cache_data(ttl=300, show_spinner=False)
def fetch_etf_vflow():
    """Fetch daily OHLCV for gold ETFs and compute volume flow metrics."""
    results = {}
...
def make_etf_vflow_chart(etf_dfs, selected_etfs=None):
    ...
    if selected_etfs is None:
        selected_etfs = [s for s in ETF_TICKERS if s in etf_dfs and s != "GLL"]
    colors = {"GLD":"#ffd700","IAU":"#58a6ff","SGOL":"#3fb950","UGL":"#ffa726","GLL":"#f85149"}
    ...
    for sym in selected_etfs:
        ...
        dash = "dash" if sym == "GLL" else "solid"
...
def fetch_news_sentiment():
    ...
    for url in feeds:
        try:
            ...
            pass
    bullish_kw = ["surge","rally","rise","gain","bull","up","high","strong","buy","support","breakout","silver"]
...
def load_snapshot():
    ...
    gs_ratio = None
    if gs_data["silver_price"] and price > 0:
        gs_ratio = round(price / gs_data["silver_price"], 2)
    # ETF flow bias
    try:
    ...
    return dict(
        ...
        silver_price=gs_data.get("silver_price"), dxy=gs_data.get("dxy"),
        yield_10y=gs_data.get("yield_10y"),
        gs_ratio=gs_ratio, silver_daily=gs_data.get("silver_daily"),
...
def get_correlation_data(period="1y"):
    tickers = {"Silver":"SI=F","USD Index":"DX-Y.NYB",
...
def check_alerts(live, snap):
    ...
    gs_ratio = snap.get("gs_ratio") or 0
    if gs_ratio > 85: alerts.append({"level":"yellow","msg":f"Gold/Silver Ratio HIGH ({gs_ratio:.1f}) — Gold historically EXPENSIVE vs Silver"})
    elif gs_ratio > 0 and gs_ratio < 55: alerts.append({"level":"green","msg":f"Gold/Silver Ratio LOW ({gs_ratio:.1f}) — Gold historically CHEAP vs Silver"})
    # ETF flow alerts
    etf_bias = snap.get("etf_flow_bias") or "neutral"
    if etf_bias == "inflow":
...
def make_chart(df, support, resistance, fib, current_price, show_ichimoku=True, timeframe="Daily",
               silver_df=None, dxy_df=None, show_gold=False, show_dxy=False):
    df = df.copy()
    if df.index.tzinfo is not None:
        df.index = df.index.tz_convert("UTC").tz_localize(None)
    rows = 4
...
def make_gs_ratio_chart(gold_df, silver_df):
    ...
    fig.add_trace(go.Scatter(x=ratio.index, y=ratio.values,
        line=dict(color="#ffd700", width=2), name="G/S Ratio"))
...
def make_dxy_overlay_chart(gold_df, dxy_df):
    gold_df = gold_df.copy()
    dxy_df = dxy_df.copy()
    if gold_df.index.tzinfo is not None:
        gold_df.index = gold_df.index.tz_convert("UTC").tz_localize(None)
    ...
    common = gold_df.index.intersection(dxy_df.index)
    if len(common) < 5:
        return None
    fig = make_subplots(specs=[[{"secondary_y": True}]])
    fig.add_trace(go.Scatter(x=common, y=gold_df.loc[common, "Close"].values,
        line=dict(color="#ffd700", width=2), name="Gold (GC=F)"),
        secondary_y=False)
    ...
    fig.update_layout(
        template="plotly_dark", paper_bgcolor="#0d1117", plot_bgcolor="#0d1117",
        height=300, title="Gold vs USD Index (Inverse Correlation)",
        legend=dict(font=dict(size=11, color="#c9d1d9")),
        hovermode="x unified", margin=dict(l=10, r=80, t=40, b=10))
    fig.update_yaxes(title_text="Gold (USD/oz)", gridcolor="#1c2128", secondary_y=False)
    fig.update_yaxes(title_text="DXY", secondary_y=True)
    return fig

def tradingview_widget(symbol="OANDA:XAUUSD",theme="dark",height=580,interval="60"):
...
def get_ai_analysis(price_key, api_key, gs_ratio=None, dxy=None, yield_10y=None, sentiment_score=None, etf_bias=None):
    ...
    if dxy:
        extra_context += f"\nDXY (USD Index): {dxy:.2f} (Gold has inverse correlation with USD)"
    if yield_10y:
        extra_context += f"\n10-Year Treasury Yield: {yield_10y:.3f}% (Higher rates = headwind for gold)"
    ...
    prompt = ("Analyze this live COMEX gold (GC=F) market snapshot and provide a complete pro-trader setup.\n\n"
              "=== PRICE ACTION ===\n"
    ...
    resp = requests.post("https://api.anthropic.com/v1/messages",
        ...
        json={"model":AI_MODEL,"max_tokens":2500,
              "system":"You are a senior COMEX gold futures trader. Give precise actionable setups. All prices USD/oz.",
...
# ══════════════════════════════════════════════════════════════════════════════
with st.sidebar:
    st.markdown("## 🥇 Gold AI Agent PRO v4")
    st.caption("COMEX Gold Futures (GC=F)")
    st.divider()
    ...
    show_dxy_overlay = st.checkbox("Show DXY vs Gold Chart", value=True)
    ...
    st.markdown("ETF Volume Flow (GLD/IAU/SGOL/UGL/GLL)")
    st.divider()
...
# MAIN PAGE
# ══════════════════════════════════════════════════════════════════════════════
st.markdown("<h1 style=color:#f0f6fc;margin-bottom:0>🥇 Gold AI Trading Agent PRO v4</h1>",
            unsafe_allow_html=True)
st.caption("Live COMEX Gold Futures (GC=F) · Updated "+datetime.now().strftime("%H:%M:%S")
...
silver_price = snap.get("silver_price")
dxy_val = snap.get("dxy")
yield_10y_val = snap.get("yield_10y")

cards = [
    ("🥇 GOLD / OZ","$"+str(p), arrow+" "+str(abs(chg))+"%", chg_color),
    ...
    ("🥈 Silver Price","$"+str(round(silver_price,2)) if silver_price else "—",
...
Historically, ratio above 80 means gold is expensive relative to silver — watch for mean-reversion.
Ratio below 55 suggests gold is undervalued vs silver — a classic contrarian entry signal.
...
with tab_tv:
    st.markdown("⁠#### 📡 TradingView Live Gold Chart")
    ...
    tv_html = tradingview_widget(symbol="OANDA:XAUUSD",theme="dark",height=tv_height,interval=tv_interval)
...
if show_gold_overlay or show_dxy_overlay:
    ...
    with col_gs:
        if show_gold_overlay and snap.get("silver_daily") is not None:
            fig_gs = make_gs_ratio_chart(snap["daily_df"], snap["silver_daily"])
            if fig_gs:
    ...
    with col_dxy:
        if show_dxy_overlay and snap.get("dxy_daily") is not None:
            ...
            st.caption("Enable 'Show DXY vs Gold Chart' in sidebar")
    st.divider()

# ── ETF Volume Flow Section ───────────────────────────────────────────────────────────────
if show_etf_vflow:
    st.markdown("### 📊 Gold ETF Volume Flow — GLD · IAU · SGOL · UGL · GLL")
    st.caption("Cumulative dollar flow, daily volume, OBV comparison · 3-month window · GLL = inverse/bear ETF")
    ...
    if etf_summary:
        ...
        with etf_tab1:
            etf_select = st.multiselect(
                ...
                default=[s for s in ["GLD","IAU","SGOL"] if s in etf_dfs],
...
with pf_tab1:
    ...
    fig_pf.add_trace(go.Candlestick(
        ...
        name='Gold',line_width=1),row=1,col=1)
...
st.markdown('### 📡 Gold Correlation with Related Assets')
with st.spinner('Fetching correlation data…'):
    try:
        corr_data = get_correlation_data('1y')
        gold_ret = snap['daily_df']['Close'].pct_change().dropna()
        corr_results = {}
        for name,ret in corr_data.items():
            aligned = pd.concat([gold_ret,ret],axis=1,join='inner')
            if len(aligned)>10:
                c_val = aligned.corr().iloc[0,1]
                corr_results[name] = round(c_val,3)
        if corr_results:
            corr_df = pd.DataFrame(list(corr_results.items()),columns=['Asset','Correlation with Gold'])
            corr_df = corr_df.sort_values('Correlation with Gold',ascending=False)
            fig_corr = px.bar(corr_df,x='Asset',y='Correlation with Gold',
                color='Correlation with Gold',color_continuous_scale='RdYlGn',
                title='Gold 1-Year Correlation with Key Assets',range_color=[-1,1])
...
st.caption('Advanced analytics and utilities to enhance your gold trading edge')
...
with val_tab1:
    st.markdown('### 📐 Gold Position Size Calculator')
    ...
    st.info(f'Risk/oz: ${risk_oz:.2f} | Pos: {pos_oz:.0f} oz | Full Contracts: {contracts_std:.2f} | Mini Contracts: {contracts_mini:.2f} | GLD Shares: {pos_oz:.0f}')

# ─── TAB 2: Seasonality
with val_tab2:
    st.markdown('### 📅 Gold Price Seasonality Analysis')
    ...
    fig_sea.update_layout(
        title='Avg Monthly Gold Returns (%) — Historical',
...
with val_tab3:
    st.markdown('### ⚡ Gold Volatility Meter')
...
with val_tab4:
    ...
    try:
        ...
        if df_1d4 is not None and len(df_1d4) > 50:
            ...
            fig_mtf4 = go.Figure()
            fig_mtf4.add_trace(go.Scatter(x=c4daily.index, y=c4daily.values, name='Gold', line=dict(color='#c0c0c0', width=2)))
...
with val_tab5:
    st.markdown('### ₿ Gold vs Crypto & Macro Correlation')
    st.caption('90-day rolling correlation with Bitcoin, Ethereum, Silver, S&P 500, and DXY')
    CORR_ASSETS5 = {'BTC-USD':'Bitcoin','ETH-USD':'Ethereum','SI=F':'Silver','DX-Y.NYB':'DXY','^TNX':'10Y Yield','^GSPC':'S&P 500'}
    try:
        with st.spinner('Fetching correlation data...'):
            ...
            if df_si5 is not None and len(df_si5) > 30:
                ...
                if corr_res5:
                    ...
                    fig_cr5.update_layout(title='90-Day Correlation with Gold', plot_bgcolor='#0d1117',
                    ...
                    gold5 = corr_res5.get('Silver', 0)
                    div_note5 = 'High BTC corr: limited diversification.' if abs(btc5)>0.5 else 'BTC diverging from Gold: good diversification.'
                    st.info(f'Portfolio: Gold-Silver = {gold5:.2f} | Gold-BTC = {btc5:.2f} | {div_note5}')
                else:
                    st.warning('Could not fetch correlation data.')
            else:
                st.warning('Gold data not available for correlation.')
    except Exception as ecrr5:
        st.warning(f'Correlation error: {ecrr5}')

# ─── TAB 6: Trade Journal
with val_tab6:
    st.markdown('### 🎯 Gold Trade Journal & Performance Tracker')
    ...
    if trades6:
        ...
        st.info('No trades logged yet. Use the form above to start tracking your gold trades!')
st.divider()
st.markdown(
    '<div style="text-align:center;color:#8b949e;font-size:0.78rem;">'
    '🥇 Gold AI Trading Agent PRO v4 · '
    'New: ETF Volume Flow (GLD/IAU/SGOL/UGL/GLL) · '
